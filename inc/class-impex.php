...
class Impex
{
  ...
  /**
   * @param string|bool $installed_version
   */
  protected function __install_data($installed_version): bool
  {
    /*
    global $wpdb;
...
\add_action(Impex::CRON_JOB_CLEANUP_NAME, function() {
  ...
  foreach ($exports as $export) {
    if(strncmp($export['name'], 'transient-', strlen('transient-')) === 0) {
  ...
  foreach ($imports as $import) {
    if(strncmp($import['name'], 'transient-', strlen('transient-')) === 0) {
